...
# docker run -d -p 80:80 -e RAILS_MASTER_KEY=<value from config/master.key> --name ledger ledger
...
# Install base packages and PostgreSQL 16 client (matching database server version)
RUN apt-get update -qq && \
    apt-get install -y curl gnupg2 lsb-release && \
    curl -fsSL https://www.postgresql.org/media/keys/ACCC4CF8.asc | gpg --dearmor -o /usr/share/keyrings/postgresql-keyring.gpg && \
    echo "deb [signed-by=/usr/share/keyrings/postgresql-keyring.gpg] http://apt.postgresql.org/pub/repos/apt $(lsb_release -cs)-pgdg main" > /etc/apt/sources.list.d/pgdg.list && \
    apt-get update -qq && \
    apt-get install --no-install-recommends -y curl libjemalloc2 libvips libpq-dev postgresql-client-16 tzdata && \
...
RUN rm -rf .git .github .vscode .idea && \
    ...
    rm -rf script/ && \
    rm -rf node_modules && \
    find lib/tasks -name "*.rake" ! -name "import_pixiu.rake" ! -name "budgets.rake" ! -name "backfill_is_refund.rake" -delete && \

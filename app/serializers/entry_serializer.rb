...
class EntrySerializer
  ...
  def self.serialize(entry, balance: nil, account_filter: nil)
  ...
  def self.serialize_batch(entries, balance_map: {}, account_filter: nil)
  ...
  def self.compute_display_type(entry, is_transfer, is_inflow, account_filter)
    if is_transfer
  ...
  def self.compute_display_amount_type(entry, is_transfer, is_inflow, account_filter)
    if is_transfer
      if account_filter.blank?
        "TRANSFER"
      else
  ...
  def self.compute_display_name(entry, is_transfer, is_inflow, account_filter)
    if is_transfer
      transfer_counterpart = compute_transfer_counterpart(entry, is_inflow)
  ...
  def self.compute_transfer_counterpart(entry, is_inflow)
    if is_inflow

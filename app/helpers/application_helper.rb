module ApplicationHelper
  ...
  def render_category_filter_tree(categories, selected_ids, use_stimulus = false)
    all_cats = @categories || categories
    children_map = all_cats.group_by(&:parent_id)
    parent_map = all_cats.index_by(&:id)

    render_tree_nodes(categories, selected_ids, children_map, parent_map, use_stimulus)
  end
  ...
  def render_tree_nodes(nodes, selected_ids, children_map, parent_map, use_stimulus = false)
    safe_join(nodes.map do |cat|
      ...
      content_tag(:label, class: "flex items-center gap-2 py-1.5 px-3 cursor-pointer category-filter-item hover:bg-surface-hover dark:hover:bg-surface-dark-hover border-b border-border dark:border-border-dark last:border-b-0",
        ...
        if use_stimulus
          checkbox_attrs[:data][:category_filter_target] = "option item"
          checkbox_attrs[:data][:action] = "change->category-filter#toggleOption"
        end
        safe_join([
          check_box_tag("category_ids[]", cat.id, selected_ids.include?(cat.id.to_s), checkbox_attrs),
          content_tag(:span, full_name, class: "text-sm text-primary dark:text-primary-dark")
        ])
      end.html_safe +
      if children.any?
        render_tree_nodes(children, selected_ids, children_map, parent_map, use_stimulus)
      else
        "".html_safe
      end
    end)

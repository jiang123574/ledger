...
**审查日期**: 2026-04-26  
**最后更新**: 2026-04-27 (PR #183/#184/#185 合并，完成大部分优化项)  
...
| 问题 | 文件位置 | 建议 | 状态 |
|------|----------|------|------|
| Controller 过于臃肿 | AccountsController (620+ 行) | 拆分更多服务对象或 Presenters | ✅ 已优化（创建 AccountsIndexService，减少到 505 行） |
| Stimulus 控制器过多 | 57 个控制器文件 | 合并相似功能控制器（如各种 chart controller） | ✅ 已优化（创建通用控制器，减少约 400 行） |
...
| 文件位置 | 问题 | 严重程度 | 状态 |
|----------|------|----------|------|
| app/models/budget.rb:22-30 | spent_amount 每次独立查询 | 中 | ✅ 已优化（preload_spent_amounts） |
| app/models/category.rb:71 | descendants 递归产生 N+1 | 中 | ✅ 已优化（使用 CTE + map） |
| app/models/category.rb:66 | ancestors 递归产生 N+1 | 中 | ✅ 已优化（使用 CTE + map） |
| app/models/budget_item.rb | refresh_for_category 逐条更新 | 中 | ✅ 已优化（batch_descendants_map） |
...
6. [x] **Budget N+1 优化** - preload_spent_amounts 批量计算模式
7. [x] **改进 CSP 策略** - Turbo 导航限制导致 nonce 不匹配，保持 unsafe-inline 合理
8. [x] **拆分臃肿 Controller** - AccountsController 创建 AccountsIndexService
9. [x] **合并 Stimulus 控制器** - 创建 line_chart/mini_line_chart 通用控制器，减少约 400 行代码 (#185)
10. [x] **API 版本化** - 已创建 api/v1/external_controller.rb
11. [x] **Category N+1 优化** - ancestors/descendants 使用 CTE + map 替代递归
...
15. [x] **提取月度统计查询** - 已有缓存策略，收益有限
16. [x] **BudgetItem 批量优化** - batch_descendants_map 单次 CTE
...
### PR #182 (已合并 2026-04-27)

| 修复项 | 文件 | 说明 |
|--------|------|------|
| 拼音选择器交互 | selectors.js | 添加 click 事件处理重复选择 |
| Plans 内联函数 | plans/index.html.erb | 移除约 250 行内联代码 |
| 分组显示 | selectors.js | 支持 groupByKey 配置 |

### PR #183 (已合并 2026-04-27)

| 修复项 | 文件 | 说明 |
|--------|------|------|
| Budget N+1 | budget.rb | preload_spent_amounts 批量计算 |
| Controller 拆分 | accounts_controller.rb | 创建 AccountsIndexService |
| Category N+1 | category.rb | ancestors 使用 map 避免递归 |

### PR #184 (已合并 2026-04-27)

| 修复项 | 文件 | 说明 |
|--------|------|------|
| BudgetItem N+1 | budget_item.rb | batch_descendants_map 批量查询 |
| Category 扩展 | category.rb | 新增 batch_descendants_map CTE 方法 |
| Controller 拆分 | accounts_controller.rb | 创建 AccountsIndexService |
| Category N+1 | category.rb | ancestors 使用 map 避免递归 |

### PR #185 (已合并 2026-04-27)

| 修复项 | 文件 | 说明 |
|--------|------|------|
| 图表控制器合并 | line_chart_controller.js | 新增通用折线图控制器 |
| 图表控制器合并 | mini_line_chart_controller.js | 新增通用迷你图控制器 |
| 别名适配 | trend_line_chart_controller.js | 继承 LineChartController |
| 别名适配 | net_worth_trend_controller.js | 继承 LineChartController (涨跌配色) |
| 别名适配 | sparkline_chart_controller.js | 继承 MiniLineChartController |
| 别名适配 | time_series_chart_controller.js | 继承 MiniLineChartController (交互模式) |
...
**审查报告已更新。已完成 14 项修复，剩余 2 项待处理（简化预算表结构、CSS 压缩）。**

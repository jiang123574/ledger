...
| 图表类型 | 用途 | 数据维度 | 实现难度 |
|----------|------|----------|----------|
| 饼图/环形图 | 分类占比 | 支出/收入按分类分布 | 低 |
| 柱状图 | 月度对比 | 各月收支总额对比 | 低 |
| 折线图 | 趋势分析 | 余额/净资产变化趋势 | 低 |
| 桑基图 | 收支流向 | 收入来源→分类→支出去向 | 高 |
| 瀑布图 | 余额变动 | 收入增加→支出减少→期末余额 | 中 |
| 日历热力图 | 消费习惯 | 每日消费金额/频率分布 | 中 |
| 堆叠柱状图 | 分类趋势 | 各月各分类支出变化 | 中 |
| 散点图 | 异常发现 | 单笔金额 vs 时间，识别大额消费 | 低 |
...
**技术栈**: 项目已使用 Chart.js，桑基图需引入 chartjs-chart-sankey 或 d3-sankey
